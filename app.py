...
def add_products():
    data = request.json
    if "name" in data and data.get("price"):
        product = Product(name=data["name"], price=data["price"], description=data.get("description", ""))
        db.session.add(product)
        db.session.commit()
        return jsonify({"message": "Product added secessfuly"})
...
def update_product(product_id):
    ...
    db.session.commit()
    return jsonify({'message': 'Product update sucessfuly'})
...
def checkout():
    ...
    for cart_item in cart_items:
        db.session.delete(cart_item)
    db.session.commit()
    return jsonify ({'message':'Checkout sucessful. Cart has been clered.'})
